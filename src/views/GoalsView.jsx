  ...
  return (
    <div>
      <div style={{padding:"34px 16px 18px"}}>
        ...
        <div style={{fontSize:13,color:"#999",marginTop:7,letterSpacing:".1em",textTransform:"uppercase"}}>track targets · stay on course</div>
      </div>

      {/* Mesocycle Phase Card */}
      <div style={{margin:"0 16px 14px",padding:"14px 16px",background:"#0d0d0d",border:`1.5px solid ${phase.color}55`,borderRadius:12,boxShadow:`0 0 24px ${phase.color}14`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontSize:10,color:phase.color,letterSpacing:".14em",textTransform:"uppercase",fontWeight:800}}>Training Phase</div>
            ...
            <div style={{fontSize:12,color:"#888",marginTop:5,lineHeight:1.5}}>{phase.hint}</div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:36,color:phase.color,letterSpacing:".04em"}}>{totalSessions}</div>
            <div style={{fontSize:10,color:"#777",letterSpacing:".1em",textTransform:"uppercase"}}>Sessions</div>
          </div>
        </div>
      </div>
      ...
      {deload.needed && (
        <div style={{margin:"0 16px 14px",padding:"12px 14px",background:"#fb718511",border:"1px solid #fb718555",borderRadius:10}}>
          <div style={{fontSize:12,color:"#fb7185",fontWeight:900,textTransform:"uppercase"}}>⚠ Deload Recommended</div>
      ...
      <Section title={`Active Goals (${activeGoals.length})`} action={<button onClick={()=>setAdding(a=>!a)} style={{padding:"6px 12px",border:`1px solid ${accent}66`,borderRadius:8,background:"transparent",color:accent,fontSize:11,fontWeight:900}}>+ NEW</button>}>
        {adding && (
          <div style={{padding:"14px",background:"#0d0d0d",border:`1px solid ${accent}44`,borderRadius:11,marginBottom:8}}>
            <div style={{display:"grid",gap:8}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <button onClick={()=>setAdding(false)} style={{padding:"12px",border:"1px solid #333",borderRadius:9,background:"transparent",color:"#888",fontWeight:800}}>CANCEL</button>
        ...
        {activeGoals.length === 0 && !adding && (
          <div style={{padding:"20px",textAlign:"center",color:"#555",fontSize:13}}>No active goals. Tap + NEW to set your first target.</div>
        )}
          ...
          <div style={{display:"grid",gap:6}}>
            {doneGoals.map(goal => (
              <div key={goal.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 13px",background:"#4ade8011",border:"1px solid #4ade8044",borderRadius:9}}>
                <div>
                  <span style={{fontSize:13,color:"#4ade80",fontWeight:800}}>✓ {goal.exerciseName || "Sessions"}</span>
                  <span style={{fontSize:12,color:"#888",marginLeft:8}}>{goal.targetValue}{goal.type==="weight"?"lb":goal.type==="reps"?" reps":" sessions"}</span>
                  {goal.note && <div style={{fontSize:11,color:"#666",marginTop:2}}>{goal.note}</div>}
                </div>
                <button onClick={()=>deleteGoal(goal.id)} style={{background:"transparent",border:"none",color:"#555",fontSize:14,padding:"4px 8px"}}>✕</button>
              </div>
            ))}
          </div>
        </Section>
...
function GoalCard({ goal, history, exConfig, totalSessions, accent, onAchieve, onDelete }) {
  ...
  return (
    <div style={{padding:"13px 14px",background:"#0d0d0d",border:`1px solid ${prog.achieved?"#4ade8066":prog.onTrack?"#1f1f1f":"#fbbf2455"}`,borderRadius:11}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
        ...
        <div style={{textAlign:"right",flexShrink:0}}>
          <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:barColor,letterSpacing:".04em"}}>
            {prog.current}<span style={{fontSize:13,color:"#888"}}>/{goal.targetValue}{typeLabel}</span>
          </div>
          {prog.daysLeft != null && <div style={{fontSize:10,color:"#666"}}>{prog.daysLeft}d left</div>}
        </div>
      </div>
      {/* Progress bar */}
      <div style={{height:6,background:"#1a1a1a",borderRadius:3,overflow:"hidden",margin:"10px 0 6px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{fontSize:11,color:"#888"}}>
          ...
          {prog.weeklyNeeded && prog.weeklyRate > 0 && (
            <span style={{color:prog.onTrack?"#4ade80":"#fbbf24",marginLeft:6}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {!goal.achieved && prog.pct >= 1 && (
            <button onClick={onAchieve} style={{padding:"5px 9px",border:"1px solid #4ade8066",borderRadius:7,background:"#4ade8022",color:"#4ade80",fontSize:10,fontWeight:900}}>MARK ✓</button>
          )}
          <button onClick={onDelete} style={{padding:"5px 8px",border:"none",borderRadius:7,background:"transparent",color:"#555",fontSize:12}}>✕</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{padding:"20px 16px 6px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:11}}>
        <div style={{fontSize:13,color:"#ddd",letterSpacing:".14em",textTransform:"uppercase",fontWeight:700}}>{title}</div>
        {action}
      </div>
      {children}
    </div>
...
function Label({ children }) {
  return <div style={{fontSize:10,color:"#888",letterSpacing:".1em",textTransform:"uppercase",fontWeight:800,marginBottom:5}}>{children}</div>;
}
